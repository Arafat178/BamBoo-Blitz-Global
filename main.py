...
API_URL = "https://bambooblitz-413ec-default-rtdb.asia-southeast1.firebasedatabase.app/scores.json"
...
async def main():
    ...
    def save_score(player_name, score_value):
        async def post_score():
            try:
                from pyodide.http import pyfetch  # এখানে import রাখতে হবে
                ...
                print("Save response:", text)  # debug
            except Exception as e:
                print("Save failed:", e)

        asyncio.create_task(post_score())

    def load_scores():
        async def fetch():
            global top_scores
            try:
                from pyodide.http import pyfetch  # এখানেও একই
                resp = await pyfetch(API_URL)
                data = json.loads(await resp.string())
                if data:
                    scores = [(v["name"], v["score"]) for v in data.values()]
                    scores.sort(key=lambda x: x[1], reverse=True)
                    top_scores = scores[:5]
                print("Loaded scores:", top_scores)  # debug
            except Exception as e:
    ...
    while running:
        ...
        if doriY >= 650:
            gameXY(100,250)
            if score_save == False:
                save_score(player_name, score_value)  # save current player score
                load_scores() #top 5 player name showing
                score_save = True
            y_offset = 620
